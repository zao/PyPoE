...
class TestPatchFileList:
    ...
    @pytest.mark.parametrize("node_path", [_TEST_FILE] + _TEST_NODE_PATHS + [_TEST_FILE])
...
@pytest.mark.skip
@pytest.mark.dependency(
    name="test_node_check_hash", depends=["test_updatelist", "test_download_file"]
)
...
def test_node_check_hash(temp, patch_file_list, recurse, node_path):
    ...
    if isinstance(node, ggpk.FileRecord) and os.path.exists(
        ...
        if recurse:
            ...
            assert len(node_hashes) == (len(node.children) - len(node.directories))


@pytest.mark.dependency(name="test_node_outdated_files", depends=["test_node_check_hash"])
@pytest.mark.parametrize(
    "recurse,node_path",
    [
        (True, _TEST_FILE),
...
def test_node_outdated_files(temp, patch_file_list, recurse, node_path):
    ...
    one_matching_file = patch_file_list.directory[_TEST_FILE] in files_in_node
