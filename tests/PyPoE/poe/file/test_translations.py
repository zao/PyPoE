...
dbase_path = os.path.join(data_dir, "Metadata", "StatDescriptions", "descriptions_base.txt")
dextended_path = os.path.join(data_dir, "Metadata", "StatDescriptions", "descriptions_extended.txt")
...
class TestTranslationFileCache:
    ...
    def test_get_file(self, tcache, dbase, dextended):
        assert (
            tcache.get_file("Metadata/StatDescriptions/descriptions_base.txt") == dbase
        ), "Files should be identical"
        assert (
            tcache.get_file("Metadata/StatDescriptions/descriptions_extended.txt") == dextended
        ), "Files should be identical"
...
class TestTranslationResults:
    ...
    functionality_tests = (
        ...
        (
            ["test_placeholder1", "test_placeholder2", "test_placeholder3", "test_placeholder4"],
            [50, 100, 150, 200],
            ["Placeholder: x y z A"],
            "placeholder",
            {
        ...
        (
            ["test_uq3_empty_value1", "test_uq3_empty_value2", "test_uq3_empty_value3"],
            [50, 100, 150],
            ["50 100 150 emp3"],
        ...
        (
            ...
            ["50 emp_value_d"],
            "Test empty formatter with no positional value and the d format " "type, i.e. {:d}",
            {},
            None,
        ),
        (
    ...
    @pytest.mark.parametrize("tags,values,result,message,kwargs,rkwargs", functionality_tests)
    def test_functionality(self, dbase, tags, values, result, message, kwargs, rkwargs):
        self.functionality(dbase, tags, values, result, message, kwargs, rkwargs)

    @pytest.mark.parametrize(
        "filename,tags,values,result,message,kwargs,rkwargs", live_functionality_tests
    )
    def test_live_functionality(
        self, ggpk_tc, filename, tags, values, result, message, kwargs, rkwargs
    ):
        self.functionality(ggpk_tc[filename], tags, values, result, message, kwargs, rkwargs)
    ...
    def test_quantifier_mod_value_to_item_class(self, tf_data, rr):
        ...
        tags = [
            "test_quantifier_mod_value_to_item_class",
        ]
        values = [
            row.rowid,
        ]
        ...
        assert trr.translations[0].ids == tags, "reverse failed incorrect tags"
        assert trr.values[0] == values, "failed reverse incorrect values"
    ...
    def test_quantifier_tempest_mod_text(self, tf_data, rr):
        ...
        tags = [
            "test_quantifier_tempest_mod_text",
        ]
        values = [
            row.rowid,
        ]
    ...
    def test_reminderstring(self, tf_data, rr):
        ...
        tags = [
            "test_quantifier_reminderstring",
        ]
        values = [
            1,
        ]
