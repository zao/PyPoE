...
spec_dir = os.path.join(os.path.split(os.path.realpath(__file__))[0], "_data", "specifications")
...
def rr_temp_dir(tmpdir_factory):
    ...
    for file_data in file_datas:
        with open(os.path.join(temp_dir, "Data", file_data["file_name"]), "wb") as temp_file:
...
def rr_instance(rr_temp_dir):
    return dat.RelationalReader(
        ...
        read_options={
            "specification": load(os.path.join(spec_dir, "rr_test.py")),
            "use_dat_value": False,
        },
    )
...
class TestDatValue:
    ...
    dv_basic_int_negative = dat.DatValue(
        value=-42, offset=0, size=4, parent=None, specification=None
    )
    dv_basic_int_positive = dat.DatValue(
        value=1337, offset=0, size=4, parent=None, specification=None
    )
    dv_basic_str_test = dat.DatValue(
        value="test", offset=0, size=4 * 2 + 4, parent=None, specification=None
    )
    dv_basic_str_a = dat.DatValue(
        value="a", offset=0, size=1 * 2 + 4, parent=None, specification=None
    )
    dv_basic_str_z = dat.DatValue(
        value="z", offset=0, size=1 * 2 + 4, parent=None, specification=None
    )
    ...
    dv_pointer.child = dat.DatValue(
        value="test", offset=8, size=4 * 2 + 4, parent=dv_pointer, specification=None
    )

    dv_pointer_int_negative = dat.DatValue(
        value=8, offset=0, size=4, parent=None, specification=None
    )
    dv_pointer_int_negative.child = dat.DatValue(
        value=-42, offset=8, size=4, parent=dv_pointer_int_negative, specification=None
    )

    dv_pointer_int_positive = dat.DatValue(
        value=8, offset=0, size=4, parent=None, specification=None
    )
    dv_pointer_int_positive.child = dat.DatValue(
        value=1337, offset=8, size=4, parent=dv_pointer_int_positive, specification=None
    )
    ...
    def test_instance_pointer(self):
        ...
        # Property Tests
        assert (
            self.dv_pointer.data_size == self.dv_pointer.child.size
        ), "Should return the size of the child"
        assert (
            self.dv_pointer.data_start_offset == self.dv_pointer.value
        ), "Should return the self.dv_pointer.value, i.e. the pointer"
        assert (
    ...
    def test_instance_list(self):
        ...
        # Property Tests
        assert (
            self.dv_list.data_size == self.dv_list.value[0] * self.dv_list.children[0].size
        ), "Should return sizeof(self) + sizeof(child)"
        assert (
            self.dv_list.data_start_offset == self.dv_list.value[1]
        ), "Should return the self.dv_list.value, i.e. the pointer"
        assert (
...
class TestSpecificationErrors:
    ...
    def test_runtime_missing_foreign_key(self, rr_temp_dir, spec_name):
        rr = dat.RelationalReader(
            ...
            read_options={
                "specification": load(os.path.join(spec_dir, spec_name)),
                "use_dat_value": False,
            },
        )
        with pytest.raises(dat.SpecificationError) as e:
            rr.get_file("Data/Main.dat")
        assert e.value.code == dat.SpecificationError.ERRORS.RUNTIME_MISSING_FOREIGN_KEY
...
class TestRelationalReader:
    ...
    def test_getitem(self, rr_instance):
        assert rr_instance["Main.dat"] == rr_instance.get_file("Data/Main.dat").reader
