...
class TestIDLRecord:
    ...
    def test_repr(self, idl_record):
        record, d = idl_record
        assert (
            eval(repr(record)) == record
        ), "repr() should return an object that would create a valid IDLRecord"
        ...
        with TemporaryDirectory() as d:
            ...
            with open(tmp_path, "rb") as f1:
                with open(idl_path_result, "rb") as f2:
                    assert (
                        f1.read() == f2.read()
                    ), "Written file should be equal to the write test file."
