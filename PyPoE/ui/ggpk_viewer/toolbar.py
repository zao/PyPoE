...
class ContextToolbar(QToolBar):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.action_extract.setStatusTip(
            self.tr("Extract currently selected file or folder")
        )
                    ...
                    for file_name in files:
                        if file_name.endswith(".dds"):
                            self._toolbar_extract_dds(
                                os.path.join(root, file_name), node
                            )
            elif isinstance(node.record, ggpk.FileRecord) and node.name.endswith(
                ".dds"
            ):
