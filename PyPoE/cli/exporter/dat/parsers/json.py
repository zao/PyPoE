...
class JSONExportHandler(DatExportHandler):
    ...
    def handle(self, args):
        ...
        with open(args.target, mode="w", encoding="ascii" if args.ascii else "utf-8") as f:
            ...
            for file_name in args.files:
                ...
                header = [
                    ...
                    for index, (name, props) in enumerate(dict_spec[file_name]["fields"].items())
                ]
                ...
                if args.use_object_format:
                    out_obj = {
                        ...
                        "data": [
                            {cid: row[i] for i, cid in enumerate(dat_file.reader.columns_data)}
                            for row in dat_file.reader.table_data
                        ],
                    }

                    virtual_header = {row["name"]: row for row in virtual_header}
                else:
                    out_obj = {
                        "filename": file_name,
                        "header": header,
