...
_inter_wiki_map = {
    ...
    "Russian": (
        ...
        (
            "(?:Очередью|Стрельбой очередями) (?:[0-9]+ уровня)",
            {"link": "Стрельба очередями"},
        ),
...
class BaseParser:
    ...
    def __init__(self, base_path, parsed_args):
        ...
        self.tc = TranslationFileCache(
            path_or_file_system=self.file_system, **self._TC_KWARGS
        )
            ...
            for i, tr in enumerate(result.found):
                try:
                    ...
                    temp_trans.insert(
                        index,
                        make_inter_wiki_links(
                            tr.get_language(self.lang).format_string(result.values[i])[
                                0
                            ]
                        ),
                    )
