...
class UniqueCommandHandler(ExporterHandler):
    def __init__(self, sub_parser):
        ...
        copy.add_argument(
            ...
            dest="en_user",
            help=(
                "Poewiki user name to use to login into the English wiki (source). "
                "Bot access speeds things up."
            ),
            action="store",
            type=str,
            default="",
        ...
        copy.add_argument(
            ...
            dest="en_password",
            help=(
                "Poewiki password to use to login into the English wiki (source). "
                "Bot access speeds things up."
            ),
            action="store",
            type=str,
...
class UniqueCopy(BaseParser):
    def __init__(self, *args, **kwargs):
        ...
        for row in self.rr_english["BaseItemTypes.dat"]:
            ...
            self.cache[row["ItemClassesKey"]["Id"]].index["Name"][row["Name"]].append(
                row
            )
    ...
    def fuzzy_find_text(
        ...
        if len(results) == 0:
            ...
            try:
                ...
                # FIXME: either use this properly or don't bother catching..
                traceback.print_exc()
    ...
    def copy(self, parsed_args, pn):
        ...
        if self.parsed_args.copy_upgraded_from:
            ...
            # Need to copy the list or it won't be deleted properly as it deletes
            # from itself during iteration
        ...
        if pn == name:
            ...
            while cont:
                ...
                cont = input("y/n?\n") != "y"
            # FIXME: `t` could be unbound, refactor..
...
def run():
    ...
    # FIXME: can't use `self` instance outside of a class..
    # What is even this? Remove if unused, this looks unfinished and old...
    for row in self.rr_english["BaseItemTypes.dat"]:
