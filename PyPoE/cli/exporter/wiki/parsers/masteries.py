...
class MasteryCommandHandler(ExporterHandler):
    def __init__(self, sub_parser, *args, **kwargs):
        ...
        mastery_eff_parser.set_defaults(
            func=lambda args: mastery_eff_parser.print_help()
        )
        ...
        mastery_grp_parser.set_defaults(
            func=lambda args: mastery_grp_parser.print_help()
        )
...
class MasteryEffectParser(parser.BaseParser):
    ...
    def by_id(self, parsed_args):
        return self.export(
            parsed_args,
            self._passive_column_index_filter(column_id="Id", arg_list=parsed_args.id),
        )
...
class MasteryGroupParser(parser.BaseParser):
    ...
    def by_id(self, parsed_args):
        return self.export(
            parsed_args,
            self._mastery_column_index_filter(column_id="Id", arg_list=parsed_args.id),
        )
            ...
            if mastery_grp[icon_field]:
                ...
                if mastery_grp[icon_field].startswith("Art/2DArt/SkillIcons/passives/"):
                    if icon[-2] == "MasteryPassiveIcons":
                        ...
                        warnings.warn(
                            f"Icon path is not as expected for {mastery_grp['Id']}"
                        )
                else:
                    data["icon"] = icon[-1]
                    pass
                ...
                data["icon"] = ""
                warnings.warn(
                    f"Icon path file not found for {mastery_grp['Id']}: {data['name']}"
                )
