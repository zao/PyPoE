...
class LuaFormatter:
    ...
    def format_module(self, data, indent=0, newline=True):
        ...
        out.append(
            "local data = %s"
            % self.format_value(data, indent=indent + 1, newline=newline)
        )
        out.append("\n")
    ...
    def main(self, parsed_args):
        ...
        for row in self.rr["BestiaryRecipeComponent.dat64"]:
            ...
            if row["BeastRarity"] != RARITY.ANY and row["BeastRarity"] is not None:
                display_string = (
                    "ItemDisplayString" + row["BeastRarity"].name_lower.title()
                )
...
class HarvestParser(GenericLuaParser):
    ...
    def main(self, parsed_args):
        ...
        for row in self.rr["HarvestCraftOptions.dat64"]:
            self._copy_from_keys(
                row, self._COPY_KEYS_HARVEST_CRAFT_OPTIONS, harvest_craft_options
            )
