...
def _colorize_rgba(
    img, black, white, mid=None, blackpoint=0, whitepoint=255, midpoint=127
):
    img_a = img.getchannel("A")
    img_gray = ImageOps.grayscale(img)

    ret = ImageOps.colorize(
        img_gray, black, white, mid, blackpoint, whitepoint, midpoint
    )
    ret.putalpha(img_a)
    return ret
...
class ItemsParser(SkillParserShared):
    ...
    def _conflict_quest_items(self, infobox, base_item_type, rr, language):
        ...
        if match:
            ...
            try:
                return (
                    base_item_type["Name"]
                    + " (%s)" % rr["Quest.dat64"].index["Id"][qid]["Name"]
                )
            except KeyError:
    ...
    def _conflict_hideout_doodad(self, infobox, base_item_type, rr, language):
        try:
            ho = rr["HideoutDoodads.dat64"].index["BaseItemTypesKey"][
                base_item_type.rowid
            ]
        except KeyError:
            return
    ...
    def _process_base_item_type(self, base_item_type, infobox, not_new_map=True):
        ...
        if description:
            infobox["description"] = self.rr["ClientStrings.dat64"].index["Id"][
                description
            ]["Text"]
        ...
        for base_item_type in items:
            ...
            if self._language != "English" and parsed_args.english_file_link:
                ...
                if icon:
                    ...
                    infobox[key] = self.rr2["BaseItemTypes.dat64"][
                        base_item_type.rowid
                    ]["Name"]
        ...
        for row, atlas_node in map_series_tiers.items():
            ...
            if parsed_args.store_images:
                ...
                # Warn about and skip maps that are on atlas but have no icon.
                    ...
                    atlas_node is not None
                    and not atlas_node["ItemVisualIdentityKey"]["DDSFile"]
                ):
                    warnings.warn(
