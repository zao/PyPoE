...
class AreaParser(parser.BaseParser):
    ...
    _COPY_KEYS = OrderedDict(
        (
            ...
            (
                ...
                {
                    "template": "entry_npc",
                    "condition": lambda area: area["FirstEntry_NPCTextAudioKey"]
                    is not None,
            ...
            (
                "VaalArea_SpawnChance",
                {
                    ...
                    "condition": lambda area: area["VaalArea_SpawnChance"] > 0
                    and area["VaalArea_WorldAreasKeys"],
                },
            ),
            (
    ...
    def export(self, parsed_args, areas):
        ...
        for area in areas:
            ...
            #
            if not parsed_args.skip_main_page:
                map = self.rr["Maps.dat64"].index["Regular_WorldAreasKey"].get(area)
                if map:
                    map = map[0]
                    ...
                    if map_version:
                        if (
                            map_version
                            == self.rr["MapSeries.dat64"].index["Id"]["MapWorlds"][
                                "Name"
                            ]
                        ):
                            map_version = None
