...
class PassiveSkillParser(parser.BaseParser):
            ...
            (
                "PassiveSkillBuffsKeys",
                {
                    ...
                    "format": lambda value: ",".join(
                        [x["BuffDefinitionsKey"]["Id"] for x in value]
                    ),
    ...
    def export(self, parsed_args, passives):
        ...
        passives = [
            passive for passive in passives if not passive["Id"].startswith("royale")
        ]
        ...
        for passive in passives:
            ...
            # atlas_start_node doesn't have an icon path
                ...
                data["icon"] = ""
                warnings.warn(
                    f"Icon path file not found for {passive['Id']}: {passive['Name']}"
                )
            ...
            for ps_buff in passive["PassiveSkillBuffsKeys"]:
                stat_ids = [
                    stat["Id"] for stat in ps_buff["BuffDefinitionsKey"]["StatsKeys"]
                ]
