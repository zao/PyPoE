...
class ItemParser:
    ...
    _re_rarity = re.compile(r"^Rarity: (?P<rarity>.*)$", re.UNICODE | re.MULTILINE)
    ...
    _stat_handlers = {
        ITEM_TYPES.ITEM: {
            ...
            "elemental_damage": {
                "re": "Elemental Damage",
                "func": lambda s: [[int(s3) for s3 in s2.split("-")] for s2 in s.split(", ")],
            },
            "chaos_damage": {
                "re": "Chaos Damage",
                "func": lambda s: [int(s2) for s2 in s.split("-")],
    ...
    _re_stat_handlers = {
        ...
        ITEM_TYPES.CURRENCY: _regex_build_from_handler_dict(_stat_handlers[ITEM_TYPES.CURRENCY]),
    }
    ...
    _re_sockets = _re_sockets_split = re.compile(r"^Sockets: (?P<sockets>.+)$", re.UNICODE)
        ...
        if self._type in self._stat_handlers:
            increment_sec(
                self._handle_handlers(
                    next, self._re_stat_handlers[self._type], self._stat_handlers[self._type]
                )
            )
