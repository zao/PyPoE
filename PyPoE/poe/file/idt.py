...
class IDTFile(AbstractFile):
    ...
    _regex_coordinates = re.compile(
        r"(?P<x>[0-9]+)[ ]+" r"(?P<y>[0-9]+)[ ]*" r"", re.UNICODE | re.MULTILINE | re.DOTALL
    )
    ...
    def _read(self, buffer, *args, **kwargs):
        ...
        for tex_match in self._regex_texture.finditer(match.group("textures")):
            ...
            if len(coordinates) != int(tex_match.group("count")):
                raise ParserError(
                    "Amount of found coordinates (%s) does not match the amount of specified"
                    " coordinates (%s)" % (len(coordinates), tex_match.group("count"))
                )
