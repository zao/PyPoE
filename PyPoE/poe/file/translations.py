...
regex_lang = re.compile(
    r'^[\s]*lang "(?P<language>[\w ]+)"[\s]*$', re.UNICODE | re.MULTILINE
)
...
class Translation(TranslationReprMixin):
    ...
    def __init__(
        self, identifier: Union[str, None] = None, tf_index: Union[int, None] = None
    ):
                ...
                if not use_placeholder:
                    ...
                    if is_range[tagid]:
                        ...
                        # TODO: how to show ranges for text stuff?
                            ...
                            range_fmt = self._RANGE_FORMAT
                        value = range_fmt.format(fmt, fmt.replace("{0", "{1")).format(
                            *value
                        )
                    else:
...
class TranslationRange(TranslationReprMixin):
    ...
    def __init__(
        self, min: int, max: int, parent: TranslationString, negated: bool = False
    ):
...
class TranslationQuantifierHandler(TranslationReprMixin):
    ...
    def init(cls):
        cls.regex = re.compile(
            r"(%s)(?!\_)" % "|".join(cls.handlers.keys()), re.UNICODE
        )
...
class TQReminderString(TranslationQuantifier):
    ...
    def handle(self, *args):
        return self.relational_reader["ReminderText.dat64"].index["Id"][
            args[0].strip()
        ]["Text"]
...
class TranslationReverseResult(TranslationReprMixin):
    ...
    def __init__(
        self, translations: List[Translation], values: List[Union[int, float]]
    ):
...
class TranslationFile(AbstractFileReadOnly):
                    ...
                    for i in range(0, int(tcount.group())):
                        ...
                        for j in range(0, id_count):
                            ...
                            if matchstr == "#":
                                ...
                                value = int(matchstr)
                                TranslationRange(
                                    value, value, parent=ts, negated=negated
                                )
                            elif "|" in matchstr:
    ...
    def reverse_translation(
        self, string: str, lang: str = "English"
...
def install_data_dependant_quantifiers(relational_reader):
    ...
    TranslationQuantifier(
        ...
        handler=lambda v: relational_reader[
    ...
    TranslationQuantifier(
        ...
        handler=lambda v: relational_reader["Data/AfflictionRewardTypeVisuals.dat64"][
            v
        ]["Name"],
        reverse_handler=_get_reverse_lookup_from_reader(
    ...
    TranslationQuantifier(
        id="passive_hash",
        handler=lambda v: relational_reader["Data/PassiveSkills.dat64"][v][
            "PassiveSkillGraphId"
        ],
        reverse_handler=_get_reverse_lookup_from_reader(
