...
class DatValue:
    ...
    * raises TypeError if one holds a list, and the other an integer
    ...
    def __init__(
        self, value=None, offset=None, size=None, parent=None, specification=None
    ):
        self.value = value
        self.size = size
        self.offset = offset
...
class DatRecord(list):
    ...
    def __repr__(self):
        stuff = [
            "{%s: %s}" % (k, self[i]) for i, k in enumerate(self.parent.table_columns)
        ]
        return "[%s]" % ", ".join(stuff)
...
class DatReader(ReprMixin):
    ...
    def _cast_from_spec(
        ...
        if casts[0][0] in (
            ...
            if self.use_dat_value:
                value = DatValue(
                    string, offset, offset_new - offset + 4, parent, specification
                )
            else:
                value = string
    ...
    def _process_row(self, rowid):
        ...
        for spec, casts in self.cast_spec:
            ...
            row_data.append(
                self._cast_from_spec(spec, casts, data=cell_data, offset=offset)
            )
            offset += casts[0][1]
            i += 1
...
class RelationalReader(AbstractFileCache):
    ...
    def __init__(
        self, raise_error_on_missing_relation=False, language=None, *args, **kwargs
    ):
    ...
    def _set_value(self, obj, other, key, offset):
        if obj is None:
            ...
            try:
                ...
                msg = (
                    'Did not find proper value for foreign key "%s" with '
                    'value "%s"' % (key, obj)
                )
    ...
    def get_file(self, file_name):
        ...
        for key, spec_row in df.reader.specification.fields.items():
            if spec_row.key:
                ...
                # read option already.
                if (
                    not self.read_options.get("auto_build_index")
                    and not key_offset
                    and key_id
                ):
